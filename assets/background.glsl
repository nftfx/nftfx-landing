...
/////// photoshop blend modes: https://github.com/jamieowen/glsl-blend
...
vec2 triPoint(vec2 p) {
    float t0 = hash12(p);
    return tri2cart*p + 0.45*randCircleSpline(p, 0.5*iTime + t0);
}
...
vec3 renderBackground_triangles(in vec2 uv, in float T) {
...
void main()
{
    ...
    gl_FragColor = vec4(renderBackground_triangles(uv, time), 1.);
} 